...
use kube::runtime::{metadata_watcher, predicates, reflector, watcher, WatchStreamExt as _};
use kube::{
    api::Api,
    client::Client,
    runtime::{
...
pub async fn run_cluster_class_controller(state: State) {
    ...
    let fleet_groups = watcher(Api::<ClusterGroup>::all(client.clone()), Config::default())
        .default_backoff()
        .modify(|cg| {
    ...
    let (reader, writer) = reflector::store();
    let cluster_classes = watcher(Api::<ClusterClass>::all(client.clone()), Config::default())
        .default_backoff()
    ...
    let cluster_class_controller = Controller::for_stream(cluster_classes, reader)
        .owns_shared_stream(
            subscriber.filter(|r| future::ready(r.meta().deletion_timestamp.is_some())),
        )
        .shutdown_on_signal()
        .run(
